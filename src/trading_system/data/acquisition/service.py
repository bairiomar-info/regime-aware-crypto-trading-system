...
def acquire_to_parquet(*, provider: HistoricalCandleProvider, instrument: Instrument, timeframe: Timeframe,
    ...
    if ordered and (ordered[0].open_time < start or ordered[-1].open_time > end):
        raise AcquisitionDataQualityError("provider returned candles outside requested bounds")
