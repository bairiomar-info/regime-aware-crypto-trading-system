...
class BinanceRateLimitError(BinanceAdapterError):
    ...
    def __init__(self, status: int, retry_after: Decimal | None) -> None:
...
class BinanceKlineClient:
    ...
    def __init__(self, *, base_url: str = "https://api.binance.com", retry_policy: RetryPolicy | None = None, transport: Transport | None = None, sleeper: Sleeper = lambda _: None, clock: Clock = lambda: datetime.now(timezone.utc)) -> None:
    ...
    def fetch(self, *, instrument: Instrument, timeframe: Timeframe, start: datetime, end: datetime) -> list[Candle]:
        ...
        while cursor_ms <= end_ms:
            ...
            for row in rows:
                ...
                if cursor_ms <= raw.open_time_ms <= end_ms:
                    candles.append(self.normalizer.normalize(raw, instrument=instrument, timeframe=timeframe, is_closed=self._to_ms(self.clock()) > raw.close_time_ms))
    ...
    def _parse_row(row: Any) -> BinanceRawKline:
        ...
        try:
            ...
            return BinanceRawKline(open_time_ms=int(values[0]), open=str(values[1]), high=str(values[2]), low=str(values[3]), close=str(values[4]), volume=str(values[5]), close_time_ms=int(values[6]), quote_volume=str(values[7]), trade_count=int(values[8]), taker_buy_base_volume=str(values[9]), taker_buy_quote_volume=str(values[10]))
        except (TypeError, ValueError, IndexError) as exc:
            raise BinanceAdapterError("Malformed Binance kline row values") from exc

    @staticmethod
    def _retry_after(headers: Mapping[str, str]) -> Decimal | None:
