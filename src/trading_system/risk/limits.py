...
def validate_order_risk(state: PortfolioState, order: OrderIntent, price: Decimal, limits: RiskLimits) -> None:
    if not price.is_finite() or price <= 0:
        raise ValueError("price must be positive and finite")
    equity = state.cash
    for balance in state.balances:
        equity += balance.quantity * price if balance.symbol == order.symbol else Decimal("0")
    if equity <= 0:
