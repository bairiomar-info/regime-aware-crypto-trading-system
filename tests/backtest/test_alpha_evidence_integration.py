...
def _summary(cases: int = 5, positive: str = "0.80", drawdown: str = "0.20") -> RobustnessSummary:
    return RobustnessSummary(cases, Decimal(positive), Decimal(drawdown))


def _regime(name: str, worst: str) -> RegimeOOSResult:
    return RegimeOOSResult(name, 2, Decimal("0.50"), Decimal(worst), Decimal(worst), Decimal("0.10"))


def test_alpha_evidence_integrates_gate_and_regime_evidence() -> None:
    evidence = evaluate_alpha_evidence(
        _summary(),
        (_regime("bear", "0.01"), _regime("bull", "0.03")),
        oos_window_count=4,
        worst_oos_return=Decimal("0.005"),
    )
...
def test_gate_rejection_remains_distinct_from_positive_oos_evidence() -> None:
    config = AlphaGateConfig(min_cases=10)
    evidence = evaluate_alpha_evidence(
        _summary(cases=5),
        (_regime("bull", "0.01"),),
        oos_window_count=2,
