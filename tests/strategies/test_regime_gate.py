...
def feature(time: datetime) -> FeatureSnapshot:
    return FeatureSnapshot(time, Decimal("0"), Decimal("0"), Decimal("0"), Decimal("0"), Decimal("0"), 1)
...
def test_gate_blocks_disallowed_trend_without_changing_strategy_signal_contract() -> None:
    ...
    context = StrategyContext(t, "BTCUSDT", features=feature(t), regime=regime(t, TrendState.DOWN))
...
def test_gate_scales_allowed_target_weight() -> None:
    t = datetime(2026, 1, 1, tzinfo=timezone.utc)
    context = StrategyContext(t, "BTCUSDT", features=feature(t), regime=regime(t, TrendState.UP))
